...
title-slide-attributes:
  data-background-image: img/jpe.jpg
  data-background-size: 15%
  data-background-position: 75% 25%
format: 
  revealjs:
    theme: _extensions/grantmcdermott/clean/clean.scss
    chalkboard: false
    logo: img/jpeicon.png
    footer: "[Reproducibility @ INSPIRE Bocconi](https://JPE-Reproducibility.github.io/INSPIRE-Bocconi/)"
    incremental: false
...
author: 
  ...
  - name: Elian Giordano
    email: eliangiordano@ub.edu
    affiliations: Institut d'Economia de Barcelona
subtitle: "[INSPIRE Bocconi](https://JPE-Reproducibility.github.io/INSPIRE-Bocconi/)"
date: last-modified
...
# Why Does This Matter? 🌍 <br> Elian Taking Over {background-color="#40666e"}

## A Growing Professional Market

* The number of journals requiring replication is growing — and so are the teams enforcing it.
* Data editor offices and dedicated replicator teams are actively being set up at major journals right now.

::: {.fragment}
:::{.callout-tip}
# Relevant for Your Academic Career

Most supervisors now require RAs to build replication packages alongside their research.
The skills covered today — Stata, R, version control, documentation — are exactly what that work demands.
:::
:::

## The AI Era Raises the Stakes

* Large language models can produce _plausible-looking_ results: tables, coefficients, even entire reference lists.

::: {.fragment}
:::{.callout-warning}
# The Hallucination Problem

* **Fabricated citations** — papers with real-sounding titles that simply do not exist.
* **Made-up statistics** — numbers consistent with the narrative but never actually computed.
* **No audit trail** — errors propagate silently; one bad paper can accumulate hundreds of downstream citations.
:::
:::

::: {.fragment}
👉 Replicators are the human check that automated tools cannot replace.
:::
...
# Inside the JPE Replication Process 🔬 {background-color="#40666e"}
...
::: {.column width="55%"}
We receive the author's replication package and run it **from scratch** in [Nuvolos](https://nuvolos.cloud) — a cloud-based Linux environment.

::: {.fragment}
::: {.callout-important}
# Clean Slate Principle
No pre-installed libraries. No author's personal setup. No shortcuts.

If the package does not reproduce on a fresh machine, it does not replicate.
:::
:::
:::
...
::: {.incremental}
* Standardised Linux environment for all replicators
* Some packages take **over a day** to run — cloud compute is essential
* Exposes path issues (`\` vs `/`) and missing dependencies immediately
:::
:::
:::
...
Before running a single line of code, we audit the **README**:

::: {.incremental}
1. **Data statement** — are all data sources identified? Are access instructions clear, and do redistribution licenses allow sharing?
2. **Computing requirements** — OS, software, exact versions. Is this feasible to replicate?
3. **Step-by-step workflow** — can an advanced graduate student follow this without contacting the authors?
:::

::: {.fragment}
:::{.callout-note}
The README is the contract between the author and the replicator. Vague language here is a red flag before we even open the data.
:::
:::

## Verification

The final step: cross-check **every** piece of output against the published manuscript.

:::: {.columns}

::: {.column width="45%"}
::: {.incremental}
* Every **figure** — layout, axis labels, values
* Every **table** — all cells, standard errors, significance stars
* Every **in-text number** — percentages, averages, test statistics
:::
:::
...
::: {.callout-warning}
# The Standard
_Exactly equal_ means exactly equal. 😬

Not "approximately right". Not "the same sign". Not "close enough given rounding".
:::
:::
:::
...
::: {.incremental}
* **Missing or incorrect data citations** — the source is mentioned in the paper but not in the README, or the link is broken.
* **Tables that do not match** — numbers in the manuscript differ from what the code actually produces.
* **Code that runs but gives different results** — often due to an undeclared package version or a platform-specific path.
* **Undocumented manual steps** — an intermediate file was created by hand and never mentioned in the README.
:::

::: {.fragment}
:::{.callout-note}
These are not rare edge cases — they are the most common reasons a replication is returned to the authors.
:::
:::
...
::: {.r-fit-text}
[**https://github.com/floswald/ReproWorkshop**](https://github.com/floswald/ReproWorkshop)
:::
...
* Follow along at [https://github.com/floswald/ReproWorkshop](https://github.com/floswald/ReproWorkshop)
