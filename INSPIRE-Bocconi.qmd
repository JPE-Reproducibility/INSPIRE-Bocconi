...
format: 
  revealjs:
    theme: [_extensions/grantmcdermott/clean/clean.scss, custom.scss]
    chalkboard: false
...
* Large value added of this event comes from your questions.
...
* Such mechanisms can reduce version conflicts amongst your dependencies. 
* To the extent that all versions of those dependencies are still available, this guarantees a stable computing environment.
...
# AI 🤖 {background-color="#40666e"}

## AI and Reproducibility

* How do researchers use AI?
* Does AI create different reproducibility concerns?
* My paper on this: [Randomness in Large Language Models](https://arxiv.org/abs/2607.24372)
* [Slides on that paper](https://floswald.github.io/publication/llm-randomness/)

## How a Model Picks the Next Word

The model writes one token at a time. Each candidate for the next token gets a probability.

![](img/llm-picks-next.png){fig-align="center" width="85%"}
...
## Four Sources of Non-Determinism

::: {.incremental}
1. **Deliberate sampling.** The next token is drawn from the score distribution, scaled by a temperature $T$. Setting $T=0$ goes greedy and costs nothing — but several 2026 reasoning models no longer expose the parameter.
2. **Silent model updates.** The provider swaps the weights behind an unchanged public name. Accuracy on one prime number task fell from 84% to 51% within three months.
3. **Floating point rounding.** Finite precision arithmetic is not associative, so the *order of operations* changes the answer. Server load, hardware and model splitting all change that order.
4. **Expert routing.** In mixture-of-experts models, per-expert capacity is limited within a batch — so whether your token reaches its preferred expert depends on *other users' requests*.
:::


## What Researchers Can Control

::: {.tight-table .tt-sources}

| What differs | In plain terms | Survives $T=0$? | Local | API |
| ---- | ------------- | :----: | :---: | :---: |
| *Deliberate randomness* | | | | |
| Sampling | Words are drawn at random from the score distribution. | **no** | full | some models |
| *The model is not the same model* | | | | |
| Silent updates | The provider swaps the model behind an unchanged name. | yes | full | none |
| *Same arithmetic, different order* | | | | |
| Server load | Other prompts arriving together change how work is divided. | yes | full | none |
| Hardware | A different chip or library divides the arithmetic differently. | yes | full | none |
| Model splitting | Partial results from several chips recombine in a varying order. | yes | partial | none |
| *A different part of the model ran* | | | | |
| Expert routing | Other users take the places, so your prompt gets its second choice. | yes | partial | none |

::: {.tight-table-caption}
Differences between two runs of an identical prompt. *Local* means open weights on the user's own hardware, *API* means the provider's infrastructure.
:::
...
::: {.tight-table-caption}
Reporting items for model generated variables. Must have items are essentially free to provide.
:::
